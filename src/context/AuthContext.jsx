...
function loadUsers(){
  try {
    return JSON.parse(localStorage.getItem('smas_users')||'[]')
  } catch(e){ return [] }
}

function saveUsers(users){
  localStorage.setItem('smas_users', JSON.stringify(users))
}

export function AuthProvider({children}){
  const [user, setUser] = useState(() => {
    try{
      return JSON.parse(localStorage.getItem('smas_current_user')||'null')
    }catch(e){ return null }
  })
